                    ...
                    if (requestBody.getType().isClassOrInterfaceType()) {
                        ...
                        switch (cdecl.getNameAsString()) {
                            ...
                            case "Long":
                                ...
                                blockStmt.addStatement(new VariableDeclarationExpr(longVariableDeclarator));
                                break;
                            case "MultipartFile":
                                ...
                                VariableDeclarator multipartVariableDeclarator = new VariableDeclarator(multipartFile, "req");
                                ...
                                multipartVariableDeclarator.setInitializer(methodCallExpr);
                                testMethod.getBody().get().addStatement(new VariableDeclarationExpr(multipartVariableDeclarator));
                                break;
                            case "Object":
                        ...
                        makeGetCall.addArgument(new NameExpr("headers"));
                    } else if (requestBody.getType().isPrimitiveType()) {
                        // Handle primitive types
                        VariableDeclarator primitiveVariableDeclarator = new VariableDeclarator(requestBody.getType(), "req");
                        primitiveVariableDeclarator.setInitializer("0"); // Default value for primitive types
                        blockStmt.addStatement(new VariableDeclarationExpr(primitiveVariableDeclarator));
                        makeGetCall.addArgument(new NameExpr("req"));
                        makeGetCall.addArgument(new NameExpr("headers"));
                    }
                    ...
                    if (returnType != null) {
                        if (returnType.isPrimitiveType()) {
                            ...
                            if (returnType.toString().equals("boolean")) {
                                blockStmt.addStatement("boolean resp = response.getBody().asBoolean();");
                                blockStmt.addStatement("Assert.assertTrue(resp);");
                            } else if (returnType.toString().equals("int")) {
                                blockStmt.addStatement("int resp = response.getBody().asInt();");
                                blockStmt.addStatement("Assert.assertNotNull(resp);");
                            } else if (returnType.toString().equals("long")) {
                                blockStmt.addStatement("long resp = response.getBody().asLong();");
                                blockStmt.addStatement("Assert.assertNotNull(resp);");
                            } else if (returnType.toString().equals("double")) {
                                blockStmt.addStatement("double resp = response.getBody().asDouble();");
                                blockStmt.addStatement("Assert.assertNotNull(resp);");
                            }
                        } else if (returnType.isClassOrInterfaceType() && returnType.asClassOrInterfaceType().getTypeArguments().isPresent()) {
                            System.out.println();
